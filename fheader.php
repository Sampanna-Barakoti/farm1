...
$_SESSION["directory"] = "http://localhost/farm1/";
...
<body>
                        ...
                        <ul class="list-inline topbar-right pull-right">
                            ...
                            <li>
                                ...
                                if ((isset($_SESSION["isLogedIn"]) && $_SESSION["isLogedIn"] == true)) { ?>
                                    <a href="<?= $_SESSION["directory"] ?>logout.php" onclick="return confirm_alert(this);">Logout (<?php if (isset($_SESSION["username"])) {
                                                                                                                                        echo ($_SESSION["username"]);
                                                                                                                                    } ?>)</a>
                                <?php    } else { ?>

                                    <a href="<?= $_SESSION["directory"] ?>login.php">Login</a> | <a href="<?= $_SESSION["directory"] ?>login.php">Register</a>
                            </li>
                        <?php } ?>
                        </ul>
                    </div>
    ...
    <script>
        setInterval(function() {
            var vid = 1;
            $.get('<?= $_SESSION["directory"] ?>customer/Addwatchlist.php', {
                ID: vid
            }, function(data) {
                $("#watchlist").html(data);
            });
        ...
        function removenoti(id) {
            $.get('<?= $_SESSION["directory"] ?>admin/notification.php', {
                RID: id
            }, function(data) {
                //alert(data);
            });
        ...
        function notiC() {
            setInterval(function() {
                var count = 0;
                $.get('<?= $_SESSION["directory"] ?>admin/notificationCount.php', {
                    Count: count
                }, function(data) {
                    $("#count").html(data);
                });
